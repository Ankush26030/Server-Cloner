...
class ServerCloner {
    ...
    sendProgress(message, progressChannel) {
        // Only try to send if channel exists and is valid
        if (progressChannel && progressChannel.isText && progressChannel.isText()) {
            progressChannel.send(message).catch((err) => {
                // Silently fail if channel doesn't exist anymore
            });
        }
    ...
    async cloneChannels(sourceGuild, targetGuild, progressChannel) {
        ...
        for (const channel of channels) {
            try {
                ...
                // Limit bitrate to 96000 for non-boosted servers
                const bitrate = channel.bitrate ? Math.min(channel.bitrate, 96000) : undefined;
    ...
    mapPermissionOverwrites(overwrites, targetGuild) {
        return [...overwrites.cache.values()].map(overwrite => {
            ...
            if (overwrite.type === 'role') {
                ...
                if (!targetId) {
                    // If role not found in mapping, try to find @everyone role
                    if (overwrite.id === overwrite.id) {
                        const everyoneRole = targetGuild.roles.cache.find(r => r.name === '@everyone');
                        targetId = everyoneRole?.id;
                    }
                }
            } else {
                // For user overwrites, keep the same user ID
                targetId = overwrite.id;
            }
            
            if (!targetId) {
                log.warning(`Could not map permission overwrite for ${overwrite.type}: ${overwrite.id}`);
                return null;
            }
